...
module.exports = {
   ...
   */
  ignoreRoutes: (app) => {
    app.config.proxyroute.ignoreRoutes = []
    ...
    app.config.routes.forEach((route) => {
      ...
      if (route.config && route.config.app && route.config.app.proxyroute && route.config.app.proxyroute.ingnore) {
        app.config.proxyroute.ignoreRoutes.push(route.path)
      }
      return
    })
    app.config.proxyroute.ignoreRoutes.reverse()
    return Promise.resolve({})
  },
  /**
   ...
   */
  alternateRoutes: (app) => {
    app.config.proxyroute.alternateRoutes = []
    ...
    app.config.routes.forEach((route) => {
      ...
      if (route.path.indexOf(':') > -1 || route.path.indexOf('*') > -1) {
        app.config.proxyroute.alternateRoutes.push(route.path)
      }
      return
    })
    app.config.proxyroute.alternateRoutes.reverse()
    return Promise.resolve({})
  }
}
